...
def get_image():
    try:
        ...
        # Get coordinates with India priority
        ...
        # Calculate date range (last 60 days for better coverage)
        to_date = datetime.utcnow()
        from_date = to_date - timedelta(days=60)
        from_iso = from_date.strftime('%Y-%m-%dT%H:%M:%SZ')
        to_iso = to_date.strftime('%Y-%m-%dT%H:%M:%SZ')

        # Define bounding box (0.05 degrees ~ 5km at equator - better for Indian cities)
